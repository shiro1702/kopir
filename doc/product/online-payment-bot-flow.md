...
| **MAX** | прямые ссылки на Т-Банк (`pay.tbank.ru/…`) | при тапе по ссылке | только webhook (без «Проверить») |
...
| `pay_check_status:{paymentId}` | проверить оплату (только TG) |
