...
**API:** исходящие запросы идут на `https://platform-api2.max.ru` (миграция до 19.07.2026). TLS — через `trusted-ca-fetch.ts` и bundle `web/certs/russian-trusted-ca-bundle.pem` (тот же, что для T-Bank).
