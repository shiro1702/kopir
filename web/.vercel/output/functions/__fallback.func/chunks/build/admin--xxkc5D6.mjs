...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(vueExports.mergeProps({ class: "min-h-screen bg-gray-50 p-6" }, _attrs))}><div class="mx-auto max-w-5xl"><div class="mb-6 flex items-center justify-between gap-4"><div><h1 class="text-2xl font-bold text-gray-900"> Kopir Admin </h1>`);
      if (vueExports.unref(adminConfig)) {
        _push(`<p class="mt-1 text-sm text-gray-500"> \u0420\u0435\u0436\u0438\u043C \u043E\u043F\u043B\u0430\u0442\u044B: ${ssrInterpolate(vueExports.unref(adminConfig).paymentModeLabel)} `);
      ...
      if (!vueExports.unref(adminSecret) || vueExports.unref(error) === "\u0412\u0432\u0435\u0434\u0438\u0442\u0435 ADMIN_SECRET") {
        _push(`<div class="mb-6 rounded-lg border bg-white p-4"><label class="mb-2 block text-sm font-medium text-gray-700"> ADMIN_SECRET </label><div class="flex gap-2"><input${ssrRenderAttr("value", vueExports.unref(adminSecret))} type="password" class="flex-1 rounded border px-3 py-2 text-sm" placeholder="Bearer secret"><button class="rounded bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700"> \u0412\u043E\u0439\u0442\u0438 </button></div></div>`);
      } else {
        _push(`<!---->`);
      }
      if (vueExports.unref(error) && vueExports.unref(error) !== "\u0412\u0432\u0435\u0434\u0438\u0442\u0435 ADMIN_SECRET") {
        _push(`<p class="mb-4 text-sm text-red-600">${ssrInterpolate(vueExports.unref(error))}</p>`);
      } else {
        _push(`<!---->`);
      }
      ...
      ssrRenderList(vueExports.unref(orders), (order) => {
        ...
        if (vueExports.unref(isTerminalMode) && !order.paymentConfirmedAt) {
          _push(`<button class="rounded bg-amber-500 px-3 py-1 text-white hover:bg-amber-600 disabled:opacity-50"${ssrIncludeBooleanAttr(vueExports.unref(confirmingId) === order.id) ? " disabled" : ""}>${ssrInterpolate(vueExports.unref(confirmingId) === order.id ? "..." : "\u041E\u043F\u043B\u0430\u0442\u0430 \u043F\u043E\u043B\u0443\u0447\u0435\u043D\u0430")}</button>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<button class="rounded bg-green-600 px-3 py-1 text-white hover:bg-green-700 disabled:opacity-50"${ssrIncludeBooleanAttr(vueExports.unref(printingId) === order.id || !canPrint(order)) ? " disabled" : ""}${ssrRenderAttr("title", !canPrint(order) ? "\u0421\u043D\u0430\u0447\u0430\u043B\u0430 \u043F\u043E\u0434\u0442\u0432\u0435\u0440\u0434\u0438\u0442\u0435 \u043E\u043F\u043B\u0430\u0442\u0443" : "")}>${ssrInterpolate(vueExports.unref(printingId) === order.id ? "..." : "\u041F\u0435\u0447\u0430\u0442\u044C")}</button></div></td></tr>`);
      });
      _push(`<!--]-->`);
